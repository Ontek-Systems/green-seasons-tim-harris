document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (menuBtn && mobileNav) {
        ...
        menuBtn.addEventListener('click', () => {
            ...
            if (!isOpen) {
                ...
                // Remove solid background only if still at top of page
                if (siteHeader && window.scrollY <= 50) {
                    siteHeader.classList.remove('scrolled');
                }
            } else {
                // X icon (Menu Open)
                svg.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>';
                // Always show solid header when nav is open
    ...
    // -- Desktop dropdowns (Services + Where We Serve) --
    function initDropdown(triggerId, menuId, caretId, isFixed) {
    ...
    initDropdown('services-trigger', 'services-mega', 'services-caret', true);
    initDropdown('where-trigger', 'where-dropdown', 'where-caret', true);
